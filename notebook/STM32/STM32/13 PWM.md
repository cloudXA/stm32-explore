...
这里使用的是 TIM4 的第3通道（TIM4_CH3）输出PWM。
...
截图保留原样，但通用表述应是“改变有效电平占空比”，而不是固定说“修改低电平时间”。本实验的LED为低电平有效，并且PWM极性配置为低，所以低电平占比越大，LED越亮；换一块高电平点亮的板子，关系可能相反。

## PWM三个量的分工

```text
PSC + ARR：共同决定PWM周期/频率
CCR：决定比较点，也就是占空比
Polarity：决定有效电平是高还是低
```

PWM Mode 1向上计数时，可先用下面的理想关系理解：
...
实际输出还要结合计数模式、PWM Mode 1/2和输出极性。CCR应限制在合理范围内；若希望严格100%占空比，不能简单把“CCR=ARR”当成绝对100%，应结合该定时器模式使用 `CCR >= ARR + 1` 或专门的强制输出策略。

## 配置与排错步骤

1. 从原理图确认PB8对应TIM4_CH3，并确认是否使用了重映射。
2. CubeMX把TIM4_CH3设为PWM Generation，配置PSC、ARR、Pulse和Polarity。
3. 初始化后必须调用 `HAL_TIM_PWM_Start(&htim4, TIM_CHANNEL_3)`。
4. 先固定测试0%、约50%、接近100%三个比较值，再写呼吸变化逻辑。
5. 若引脚无波形，检查GPIO是否为复用推挽输出、通道是否正确、定时器是否启动。
6. 若频率不对，重新确认TIM4时钟不是简单等于PCLK1。
