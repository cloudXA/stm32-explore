...
在 `main.c` 中实现 `HAL_UART_RxCpltCallback()`。回调不是固定“一次一个字符”；它会在本次 `HAL_UART_Receive_IT()` 请求的指定长度全部接收完成后被HAL调用。本实验把长度设为1，所以表现为每收到1字节回调一次。
...
线路噪声、参数不一致或接收不及时可能产生ORE、FE、NE等错误。只写完成回调而不处理错误，某些情况下接收会停住。至少应记录错误并重新启动接收：
...
具体HAL版本对错误标志的清除方式可能不同；若仍无法恢复，应查看 `huart->ErrorCode` 和当前F1 HAL实现，而不是在回调中反复盲目重启。

## 共享数据的边界

- `volatile` 只要求编译器每次真正读写变量，不保证多个操作整体原子，也不自动保护数组。
- 中断写缓冲区、主循环读缓冲区时，要明确“谁拥有这块缓冲区”和“何时允许复用”。
- 当前做法在一行完成后暂停重新接收，结构简单，但主循环处理过慢时会丢失随后到达的字节。
- 更连续的方案是双缓冲、环形缓冲或DMA+IDLE；无论哪种方案，都仍需处理粘包、拆包和缓冲区溢出。

## 调试步骤

1. 先确认阻塞接收正常，排除接线和波特率问题。
2. 在初始化后调用一次 `HAL_UART_Receive_IT()`。
3. 在回调中翻转调试GPIO，确认回调确实进入。
4. 确认回调末尾重新预约下一次接收。
5. 连续快速发送超过缓冲区的数据，观察溢出策略是否明确。
6. 人为改错波特率，观察 `HAL_UART_ErrorCallback()` 和错误计数。
