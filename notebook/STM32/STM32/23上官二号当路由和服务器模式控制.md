## ESP8266作为TCP Server，让PC作为TCP Client连接（当前代码仍是Wi-Fi STA，不是路由器）
...
- SoftAP：创建Wi-Fi热点，作用更接近无线接入点；是否还能提供互联网转发/NAT要看固件功能，不能直接等同于家用路由器。
...
可以这么理解：当前代码已经具备 RTOS 的一些“原始思想”，但它本身不是 RTOS，更接近一个手写的简易事件驱动系统，也叫“前后台系统”或“裸机 Super Loop”。
...
| 当前代码 | 类似的 RTOS 概念 |
|---|---|
| USART1 接收中断 | RTOS 中的硬件中断 ISR |
| `while(1)` | 简易任务调度循环 |
| `LED_COMMAND_PENDING` | 事件标志/二值信号量 |
| UART2 环形缓冲区 | 消息队列/Stream Buffer |
| `LED_ProcessPending()` | LED 任务 |
| `UART2_ForwardPending()` | 日志输出任务 |
| `__disable_irq()` | 临界区保护 |
| 字节解析状态机 | 通信协议任务的内部状态 |
| `HAL_Delay()` | 类似任务延时，但会阻塞整个裸机主循环 |
...
这些函数按顺序执行。假设 `LED_ProcessPending()` 等待 ESP8266 回复 5 秒，那么后面的其他业务也可能被推迟。
...
1. ESP-AT的TCP Server要求 `CIPMUX=1`，并使用连接编号；它与单连接透明传输互斥。
2. `+IPD,<id>,<len>:<payload>` 的头和Payload都可能被UART分段，多个帧也可能连续到达，解析器必须逐字节推进，不能只用一次 `strstr()` 猜完整帧。
3. `AT+CIPSEND=<id>,<len>` 中的长度是Payload字节数，不含命令本身；收到 `>` 后才能发Payload，并等待 `SEND OK` 或错误。
4. PC与ESP处于同一Wi-Fi仍可能被“AP客户端隔离”阻断。连接不上时同时检查ESP的STAIP、子网、防火墙和路由器隔离设置。
5. TCP Server只是监听端口，不提供应用鉴权。局域网内任何可连接设备都可能发送 `led1`，实际产品至少要增加身份校验、命令格式和权限控制。
6. 启动时发送 `+++` 只有满足固件规定的保护时间才是可靠退出序列；恢复流程应等待响应并校验AT模式，而不是发送后立即假定成功。

## 当前项目的准确角色总结

```text
ESP8266 Wi-Fi身份：STA，连接现有路由器
ESP8266 TCP身份：Server，监听8880
PC Wi-Fi身份：同一路由器下的终端
PC TCP身份：Client，主动连接ESP的STAIP:8880
```

如果改成SoftAP，变化的是Wi-Fi接入方式；TCP Server/Client角色可以保持不变。SoftAP创建热点不等于自动成为能访问互联网的完整路由器。

比如当前代码不断执行：

```c
if (LED_COMMAND_PENDING)
{
    ...
}
```
...
没有 LED 命令时，任务不占用 CPU；中断把命令放入队列后，RTOS 自动唤醒 LED 任务。

所以更准确地说：

> 当前代码不是一个简易 RTOS，而是自己实现了 RTOS 中一部分机制的裸机版本：中断产生事件、标志位传递事件、环形缓冲区传递数据、主循环负责调度业务。
...
理解当前项目后再学习 RTOS，会更容易理解队列、信号量和任务调度究竟解决了什么问题。
